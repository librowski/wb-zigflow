...
// Records the pre-change state; called right before every tracked mutation.
...
// Drag emits a change-stream, not one mutation: capture the state at drag
// start and commit it as a single history entry when the drag ends.
...
export function undo() {
  ...
  if (pastState) {
    ...
    useUndoRedoStore.setState((current) => ({
      past: current.past.slice(0, -1),
      future: [...current.future, { nodes, edges, layoutDirection }],
    }));
...
export function redo() {
  ...
  if (futureState) {
    ...
    useUndoRedoStore.setState((current) => ({
      past: [...current.past, { nodes, edges, layoutDirection }],
